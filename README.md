...
pairs, which are parsed as the code block's metadata:

~~~
```html title="A dumb example"
<h1>Hello, world!</h1>
```
~~~

The [unist] node objects returned by this library are "enhanced" with the
following keys:

* `lang` contains only the first "word" of the info string
* `meta` is an object of key/value pairs parsed from the remainder of the info
  string
* `title` is the title of the code block, as determined by [this
  ...
  nodes](https://github.com/syntax-tree/mdast#code) generated by [remark], and
  the [unist] documentation for more on the [underlying
  structures](https://github.com/syntax-tree/unist#unist-nodes).
...
1. If a `title` key is found in the block's `meta` key, use that.
...
1. If no previous heading is found, provide a title that describes where it
   ...
   You can, of course, modify the titles as you see fit:



## API

1. [Parse a file](#fromfile-filename--options)
